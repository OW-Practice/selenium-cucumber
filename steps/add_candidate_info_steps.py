from behave import *
from pages.add_candidate_info_page import CandidateInfoPage
import random
...
@when(u'Enter candidate details in to fields')
def enter_candidate_details_in_to_fields(context):
    context.candidate_info = CandidateInfoPage(context.driver)
    for row in context.table:
...
@then(u'Click on yes button in confirmation popup "{yes_loc}"')
